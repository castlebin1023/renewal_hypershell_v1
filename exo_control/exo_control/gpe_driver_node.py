...
class LowPassFilter:
    def __init__(self, f_c: float, dt: float):
        self.tau = 1.0 / (2.0 * math.pi * f_c)
        self.dt = dt
        self.alpha = self.dt / (self.tau + self.dt)
        self.prev_vf = None

    def update(self, v_raw: float) -> float:
        if self.prev_vf is None:
            self.prev_vf = v_raw
            return v_raw
        
        vf = self.alpha * v_raw + (1.0 - self.alpha) * self.prev_vf
        self.prev_vf = vf
        return vf
...
class MotorDriverNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("acc_topic",  "/motor/acc")

        self.declare_parameter("channel", "can0")
        self.declare_parameter("controller_id", 0x00)
        self.declare_parameter("upload_id", 0x00002900)
        ...
        self.acc_topic   = str(self.get_parameter("acc_topic").value)
        ...
        if self.enable_csv_logging:
            self.csv_file = open("/home/castlebin/robot_ws/data/gait_log.csv", "w", newline="")
            ...
            self.get_logger().info("CSV logging ENABLED → /home/castlebin/robot_ws/gait_log.csv")
        else:
            self.csv_file = None
        ...
        # ---------- filters ----------
        self.pos_filter = LowPassFilter(f_c=6.0,  dt=self.rx_period)
        self.vel_filter = LowPassFilter(f_c=4.0,  dt=self.rx_period)
        self.acc_filter = LowPassFilter(f_c=4.0, dt=self.rx_period)
        self.latest_acc: float = 0.0

        # ---------- differentiation state ----------
        self.prev_f_vel_joint_rad_s: Optional[float] = None
        self.prev_rx_time_sec: Optional[float] = None
    ...
    def parse_upload(self, msg: can.Message) -> Optional[MotorState]:
        ...
        f_pos_joint_deg = self.pos_filter.update(pos_joint_deg)
        pos_joint_rad = math.radians(f_pos_joint_deg)

        motor_mech_rpm = vel_motor_erpm / self.pole_pairs
        motor_mech_rad_s = motor_mech_rpm * 2.0 * math.pi / 60.0

        # ---------- filtered velocity ----------
        f_vel_joint_rad_s = self.vel_filter.update(motor_mech_rad_s) / self.gear_ratio
        f_vel_joint_angle = math.degrees(f_vel_joint_rad_s)

        # ---------- CAN-receive-time-based acceleration ----------
        now_sec = self.get_clock().now().nanoseconds * 1e-9

        if self.prev_f_vel_joint_rad_s is None or self.prev_rx_time_sec is None:
            acc_joint_rad_s2_raw = 0.0
        else:
            dt = now_sec - self.prev_rx_time_sec
            if dt <= 1e-6 or dt > 0.1:          # guard against abnormal dt
                acc_joint_rad_s2_raw = 0.0
            else:
                acc_joint_rad_s2_raw = (f_vel_joint_rad_s - self.prev_f_vel_joint_rad_s) / dt

        self.prev_f_vel_joint_rad_s = f_vel_joint_rad_s
        self.prev_rx_time_sec = now_sec

        self.latest_acc = self.acc_filter.update(acc_joint_rad_s2_raw)

        return MotorState(
            pos_motor_deg=pos_motor_deg,
            pos_joint_deg=pos_joint_deg,
            pos_joint_rad=pos_joint_rad,
            vel_motor_erpm=vel_motor_erpm,
            vel_joint_rad_s=f_vel_joint_rad_s,
            vel_joint_deg_s=f_vel_joint_angle,
            cur_a=cur_a,
            temp_c=temp_c,
            err=err,
        )
    ...
    def publish_state(self) -> None:
        ...
        acc_msg = Float32()
        acc_msg.data = float(self.latest_acc)
        self.acc_pub.publish(acc_msg)

        # CSV logging (init_done 이후부터만 도달)
        if self.enable_csv_logging and self.csv_writer is not None:
            now_sec = self.get_clock().now().nanoseconds * 1e-9
            self.csv_writer.writerow([
                now_sec,
                float(s.pos_joint_rad),
                float(s.vel_joint_rad_s),
                float(self.latest_acc),
            ])
            self.csv_file.flush()
